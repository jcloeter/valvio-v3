...
export default function QuizHistoryTable() {
    ...
    // @ts-ignore
    const {data: quizAttemptData, isError: isQuizAttemptDataError, isLoading: isQuizAttemptDataLoading, error, refetch} = useGetQuizAttemptCollectionByUserQuery(authSlice.uid);
    ...
    useEffect(()=>{
        ...
        completedQuizAttempts.map((qa: QuizAttempt)=>{
            ...
            // @ts-ignore
            const date = new Date(qa.completedAt).toLocaleDateString('en-US', {year: 'numeric', month: '2-digit', day: '2-digit'});;
            console.log(date);
            rowArray.push(createData(quiz.level, quiz.name, qa.score, qa.secondsToComplete, quiz.length, date.toString()))
        });
    ...
    if (isQuizDataLoading || isQuizAttemptDataLoading) {
        return <> <br/> <CircularProgress/> </>
    }

    if (isQuizDataError || isQuizAttemptDataError) {
        return <> <br/> <h3>There was an error fetching your data</h3> </>
    }

    if (rows.length === 0) {
        return <> <br/> <h3>No saved data yet. Create an account and complete quizzes to see stored data.</h3> </>
    }
                    ...
                    <TableBody>
                        {rows.map((row, i) => (
                            ...
                            >
                                ...
                                {(!row.speed || !row.length) ? <TableCell align="right">"error"</TableCell> : <TableCell align="right">{(row.speed/row.length).toFixed(2)}</TableCell>}
