...
const CorrectAnswerFeedback = () => {
    ...
    if (quizSlice.isTransposition && previousPitchTransposed) {
      message = `Correct, ${previousPitch.noteLetter}${getAccidentalSignFromName(previousPitch.accidental)} 
                transposed ${quizSlice.transpositionDescription} is ${previousPitchTransposed.noteLetter}${getAccidentalSignFromName(previousPitchTransposed.accidental)}, which is played with
                ${previousPitchTransposed.position}`;
      transposedPitchImage = <img  className = {styles["pitch-image"]} alt = "previous-pitch-image-transposed" src={createImageUrlFromPitchId(previousPitchTransposed.id)}/>;
    }else {
        message = `Correct, ${previousPitch.noteLetter}${getAccidentalSignFromName(previousPitch.accidental)} is played with ${previousPitch.position}`;
    }
